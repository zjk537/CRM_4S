...
{/// <summary>
    ...
    public sealed partial class MySqlHelper
    {
        ...
        #region AssignParameter

        private static void AssignParameterValues(MySqlParameter[] commandParameters, IDictionary<string, object> parameterDict)
        {
            if ((commandParameters == null) || (parameterDict.Count == 0))
            {
        ...
        public static int ExecuteNonQuery(string connectionString, string spName, IDictionary<string, object> parameterDict)
        {
            ...
            if ((parameterDict != null) && (parameterDict.Count > 0))
            {
                ...
                AssignParameterValues(commandParameters, parameterDict);
        ...
        public static MySqlDataReader ExecuteDataReader(string connectionString, string spName, IDictionary<string, object> parameterDict)
        {
            if (connectionString == null || connectionString.Length == 0) throw new ArgumentNullException("connectionString");
            if (spName == null || spName.Length == 0) throw new ArgumentNullException("spName");
            ...
            if ((parameterDict != null) && (parameterDict.Count > 0))
            {
                ...
                //foreach (KeyValuePair<string, object> param in parameterDict)
                //{
    ...
    public sealed class MySqlHelperParameterCache
    {
        ...
        private static MySqlParameter[] DiscoverSpParameterSet(MySqlConnection connection, string spName, bool includeReturnValueParameter)
        {
            ...
            List<MySqlParameter> tempParameters = new List<MySqlParameter>();

            //cmd.Parameters.CopyTo(discoveredParameters, 0);

            // Init the parameters with a DBNull value
            foreach (MySqlParameter cmdParam in cmd.Parameters)
            {
                if (!includeReturnValueParameter && cmdParam.Direction == ParameterDirection.Output)
                {
                    continue;
                }
                cmdParam.Value = DBNull.Value;
                tempParameters.Add(cmdParam);
            }
